...
async def test_observatory_post_declares_json_content_type() -> None:
    """Évite le 415 observé quand aiohttp envoie un POST vide sans Content-Type."""

    response = MagicMock()
    response.raise_for_status = MagicMock()
    response.json = AsyncMock(return_value={"grade": "F", "score": 10})
    ...
    with patch("axes.security.aiohttp.ClientSession", return_value=session_context):
        result = await _fetch_observatory("content-type-regression.example")

    assert result["grade"] == "F"
    session.post.assert_called_once_with(
        "https://observatory-api.mdn.mozilla.net/api/v2/scan",
        params={"host": "content-type-regression.example"},
        json={},
    )
...
class TestScan:
    ...
    async def test_scan_observatory_error_keeps_header_evidence(self) -> None:
        """Une API externe en panne ne détruit pas l'observation locale des en-têtes."""
        err_msg = (
        ...
        with (
            ...
            patch("axes.security._fetch_headers", new_callable=AsyncMock, return_value={}),
        ):
            result = await scan("https://nonexistent.invalid")
        assert result.coverage == 0.4
        assert result.score == 0.0
        assert "Observatory indisponible" in result.limitations[0]

    async def test_both_security_sources_fail(self) -> None:
        with (
            patch(
                "axes.security._fetch_observatory",
                new_callable=AsyncMock,
                side_effect=RuntimeError("observatory down"),
            ),
            patch(
                "axes.security._fetch_headers",
                new_callable=AsyncMock,
                side_effect=RuntimeError("target down"),
            ),
            pytest.raises(RuntimeError, match="Sources sécurité indisponibles"),
        ):
